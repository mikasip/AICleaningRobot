...
class Area():
    ...
    def generate_obstacles(self, amount = 0.2):
...
class CleaningRobot():
    ...
    def nearest_blocks(self):
        blocks = np.zeros(shape=(19,19))
        for i in range(19):
            for j in range(19):
                block = (self.pos[0] - 9 + i, self.pos[1] - 9 + j)
                if block in self.area.obstacles:
                    blocks[j,i] = 1
                elif block in map(lambda o: o.pos, self.population.cleaning_robots):
                    blocks[j,i] = -1
                elif block in map(lambda o: (o.x, o.y), self.population.squares_visited):
                    blocks[j,i] = 0.6
                elif block in map(lambda o: (o.x, o.y), self.population.unknown_squares):
                    blocks[j,i] = 0.2
...
def blit_map(display_surface, population, area, background, visited_img, dirt_img, obstacle_img, score_font, agent, episode, score):
    ...
    score2_surface = score_font.render("Score: " + str(score), True, (255, 0, 0))
...
def main():
    ...
    scores = []
    eps_history = []
    agent = Agent(lr = lr, gamma=0.88, epsilon=1, 
                input_dims=[num_agents,19,19,1],
                n_actions=4, mem_size=30000, batch_size=128,
                eps_end=0.001, replace = 512, epsilon_dec = 0.0003,
    ...
    for i in range(n_games):
        ...
        while move < MAX_MOVES:
            ...
            if frame_clock == 0:
                ...
                for robot in population.cleaning_robots:
                    robot.observation = robot.nearest_blocks()
            
            rewards = []
            for robot in population.cleaning_robots:
                if i*500 + move < 0:
                    direction = robot.nearest_unknown()[2]
                    if direction == (0,1):
                        action = 1
                    elif direction == (-1,0):
                        action = 2
                    ...
                    observations = (robot.observation,)
                    for robot2 in population.cleaning_robots:
                        if robot2 == robot: continue
                        observations = observations + (robot2.observation,)
                ...
                if action == 1:
                    ...
                    direction = (1,0) #right
                rewards.append(robot.perform_action(direction))
            ...
            for robot in population.cleaning_robots:
                robot.new_observation = robot.nearest_blocks()
            #print(population.cleaning_robots[0].previous_frames.astype('float32'))

            for robot in population.cleaning_robots:
                observations = (robot.observation,)
                new_observations = (robot.new_observation,)
                for robot2 in population.cleaning_robots:
                    if robot2 == robot: continue
                    observations = observations + (robot2.observation,)
                    new_observations = new_observations + (robot2.new_observation,)
                observations = np.stack(observations, axis=-1)
                new_observations = np.stack(new_observations, axis=-1)
                if num_agents == 1:
                    observations = np.expand_dims(observations, axis=2)
                    new_observations = np.expand_dims(new_observations, axis=2)
                agent.store_transition(observations, action, total_reward, new_observations, len(area.dirt) == 0 or move >= MAX_MOVES)
                agent.learn()
